...
class Conversation:
    ...
    def rolling_convo(self, user_input, found_db_texts=None):
        self.messages.append({"role": "user", "content": user_input})

        if found_db_texts is not None:
            self.messages.append({"role": "system", "content": f"Some helpful knowledge: {found_db_texts}"}) #todo does it work?

        chat_response = self.call_api() # use new added user_input to call API again
        print(f"VIP Guide: {chat_response}")
